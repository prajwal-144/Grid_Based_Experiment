"""Conservative fixed-SIS + FITS-PSF training after the Stage D/E diagnosis.

Key changes: metadata-validated mappings, row-normalized backward source,
arc-balanced normalized losses, zero-baseline skill, delayed magnification
regularization, and no self-referential source-cycle loss.
"""
...
def arc_balanced_weights(target, threshold_fraction=0.08, arc_boost=20.0, background_weight=0.05):
...
def laplacian(x):
    ...
    return F.conv2d(x, kernel.view(1, 1, 3, 3).repeat(x.shape[1], 1, 1, 1), padding=1, groups=x.shape[1])
...
def physics_scale(epoch, delay, ramp):
    ...
    if ramp <= 0:
        return 1.0
    return min(1.0, (epoch - delay + 1) / ramp)


def parse_args():
    p = argparse.ArgumentParser()
    p.add_argument("--exp-name", default="fixed_sis_corrected")
    ...
    p.add_argument("--src-cons-weight", type=float, default=0.0, help="must remain zero until distinct image branches are implemented")
    p.add_argument("--cuda", action=argparse.BooleanOptionalAction, default=True)
    args = p.parse_args()
    if args.src_cons_weight != 0:
        p.error("The old source-cycle/multi-pixel consistency is intentionally disabled; use --src-cons-weight 0")
...
def load_mappings(args, device):
    ...
    for role, filename in names.items():
        out[role], _ = load_mapping_bundle(Path(args.mapping_dir) / filename, device=device, expected={**expected, "mapping_role": role})
    return out
...
def run_epoch(model, loader, optimizer, training, epoch, args, lens, mappings, backward_avg, psf, info_hr):
    ...
    with context:
        for lr_image in tqdm(loader, desc=("train" if training else "val") + f" {epoch + 1}"):
            ...
            pred_hr = apply_psf(intrinsic_hr, psf)
            pred_lr = F.interpolate(pred_hr, size=lr_image.shape[-2:], mode="area")
...
def main():
    ...
    mappings = load_mappings(args, device)
